...
function Item2({ data, index, l, color, select, arr, bc, border, txtColor, contrastBg }) {
    return (
        <View
            key={index}
...
function PlaylistSong(props) {
    ...
    const { txt, header, bg, bg2, border, contrastBg } = props.theme;
    ...
    const Interstitial_deletePlaylist = (id) => {
        const { InterstitialAdPlacementId, current_advertise_type } = adConfig;

        if (current_advertise_type === 0) {
            InterstitialAdManager.showAd(InterstitialAdPlacementId)
                .then(() => dispatch(deletePlaylistSong(id)))
                .catch(() => dispatch(deletePlaylistSong(id)));
        } else if (current_advertise_type === 1) {
            AdMobInterstitial.showAd()
                .then(() => dispatch(deletePlaylistSong(id)))
                .catch(error => {
                    dispatch(deletePlaylistSong(id))
                    console.warn(error)
                });
        } else {
            dispatch(deletePlaylistSong(id))
    ...
    const RightActions = ({ progress, dragX, id }) => {
        ...
        return (
            <>
                <TouchableOpacity
                    onPress={() => Interstitial_deletePlaylist(id)}
                    style={{ height: 70 }}>
                    <View
                        style={{
                            flex: 1,
                    ...
                    <>
                        {selectarr.length > 0 && (
                            <TouchableOpacity
                                ...
                                onPress={() => Interstitial_deleteAll()}>
                                <Menu
                                    name="check"
                                    color={bg}
                                    size={30}
