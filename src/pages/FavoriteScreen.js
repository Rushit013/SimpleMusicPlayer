...
function Item2({ data, index, select, arr, bc, border, txtColor, contrastBg }) {
    return (
        <View
            key={index}
...
function FavoriteScreen(props) {
    ...
    const { bg, bg2, txt, border, header, contrastBg } = props.theme;
    ...
    const Interstitial_onDeleteFavorite = (id) => {
        const { InterstitialAdPlacementId, current_advertise_type } = adConfig;

        if (current_advertise_type === 0) {
            InterstitialAdManager.showAd(InterstitialAdPlacementId)
                .then(() => dispatch(deleteFavorite(id)))
                .catch(() => dispatch(deleteFavorite(id)));
        } else if (current_advertise_type === 1) {
            AdMobInterstitial.showAd()
                .then(() => dispatch(deleteFavorite(id)))
                .catch(error => {
    ...
    const RightActions = ({ progress, dragX, id }) => {
        ...
        return (
            <>
                <TouchableOpacity
                    onPress={() => Interstitial_onDeleteFavorite(id)}
                    style={{ height: 70 }}>
                    <View
                        style={{
                            flex: 1,
