...
function Item2({ data, index, arr, select, bc, border, txtColor, contrastBg }) {
    return (
        <View
            key={index}
            ...
            <View style={styles.select}>
                {select ? (
                    <View
                        style={{
                            backgroundColor: arr.includes(data.id) ? contrastBg : '#ecf1f7',
                            width: 20,
                            height: 20,
                            borderRadius: 50,
...
class SearchScreen extends React.PureComponent {
    ...
    Interstitial_startRecognition = (e) => {
        const { InterstitialAdPlacementId, current_advertise_type } = this.state.adConfig;

        if (current_advertise_type === 0) {
            InterstitialAdManager.showAd(InterstitialAdPlacementId)
                .then(() => this._startRecognition.bind(this))
                .catch(() => this._startRecognition.bind(this));
        } else if (current_advertise_type === 1) {
            AdMobInterstitial.showAd()
                .then(() => this._startRecognition.bind(this))
                .catch(error => this._startRecognition.bind(this));
        } else {
    ...
    getAdResponse = async () => {
        const { adResponse } = this.props.adSettings;
        const response = await getAdKey(adResponse);
        this.setupAdmobIntrestrial(response)
        this.setState({ adConfig: response })
    }
    ...
    render() {
        ...
        const { txt, txt2, bc, ph, header, bg, bg2, border, contrastBg } = this.props.theme;
                ...
                <View
                    ...
                    {select ? (
                        ...
                        <TouchableOpacity
                            ...
                            onPress={this.Interstitial_startRecognition.bind(this)}>
                            <Iconn
                                name="mic-none"
                                color={bg}
                                size={23}
                                            ...
                                            item.id !== '' && (
                                                <>
                                                    {index % 10 == 0
                                                        ?
                                                        <>
                                                            <BannerAd theme={this.props.theme} adConfig={this.state.adConfig} />
                                                            <TouchableOpacity
